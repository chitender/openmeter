...
func (f RouteFilter) Normalize() (RouteFilter, error) {
	if f.Currency == "" && f.Source.IsAbsent() && f.TaxCode.IsAbsent() && f.Features.IsAbsent() && f.CostBasis.IsAbsent() && f.CreditPriority == nil && f.TransactionAuthorizationStatus == nil && f.TaxBehavior.IsAbsent() {
		return f, nil
	}
